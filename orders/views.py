...
from gmain.models import ProductSize
from django.shortcuts import get_object_or_404
from decimal import Decimal
from payment.views import create_stripe_sheckout_session
...
class CheckoutView(CartMixin, View):
    ...
    def post(self, request):
        ...
        if not form_data.get('email'):
            form_data['email'] = request.uesr.email
        ...
        if form.is_valid():
            ...
            try:
                if payment_provider == 'stripe':
                    checkout_session = create_stripe_sheckout_session(order, request)
                    cart.clear()
                    if request.headers.get('HX-Request'):
                ...
                context = {
                    'form': form,
                    'cart': cart,
                    'cart_items': cart.item.select_related('product', 'product_size__size').order_by('-added_at'),
                    'total_price': total_price,
            ...
            context = {
                ...
                'cart_items': cart.item.select_related('product', 'product_size__size').order_by('-added_at'),
                'total_price': total_price,
                'error_message': f"Please coreect the errors on the form.: {str(e)}",
            }
            if request.headers.get('HX-Request'):
                return TemplateResponse(request, 'orders/checkout_content.html', context)
            return render(request, 'orders/checkout.html', context)